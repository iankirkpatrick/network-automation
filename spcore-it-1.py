...
#------------------------------------------------------------------------------
def read_interfaces( interfaces_filename ):

    interfaces = {}  # create our dictionary for storing interfaces and their info

    with open( interfaces_filename ) as interfaces_file:

        for interface_line in interfaces_file:

            interface_info = interface_line.strip().split(',')  #extract interface info from line

            interface = {'int_key': interface_info[0],
                      'int_hostname': interface_info[1],
                      'interface_id':   interface_info[2],
                      'int-ipaddr':   interface_info[3],
                      'int-subnet':   interface_info[4]}  # create dictionary of device objects ...

            interfaces[interface['int_key']] = interface  # store our device in the devices dictionary
                                                # note the key for devices dictionary entries is ipaddr
...
#------------------------------------------------------------------------------
def interface_config_worker( device, creds, interfaces ):
...
interfaces = read_interfaces( 'core_interfaces' )
...
for ipaddr,device in devices.items():
    ...
    print 'Getting interfaces configuration for: ', device
#    config_worker( device, creds[ipaddr] )
    interface_config_worker( device, creds[ipaddr], interfaces)
